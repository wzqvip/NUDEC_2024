...
void Read_DMP(void)
{
    ...
    if (sensors & INV_WXYZ_QUAT)
    {
        ...
        Pitch = asin(-2 * tmp[1] * tmp[3] + 2 * tmp[0] * tmp[2]) * 57.3;                                                                    // 计算出俯仰角
        Roll = atan2(2 * tmp[2] * tmp[3] + 2 * tmp[0] * tmp[1], -2 * tmp[1] * tmp[1] - 2 * tmp[2] * tmp[2] + 1) * 57.3;                     // 计算出横滚角
        ...
        // if (-1 < q0 && q0 < 1 && -1 < q1 && q1 < 1 && -1 < q2 && q2 < 1 && -1 < q3 && q3 < 1)
        // {
