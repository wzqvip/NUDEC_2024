...
void Set_PWM(int pwma,int pwmb)
{
	...
	if(pwma>0) 
	{
		DL_GPIO_setPins(AIN1_PORT,AIN2_PIN_13_PIN);
		DL_GPIO_clearPins(AIN2_PORT,AIN1_PIN_12_PIN);
		DL_Timer_setCaptureCompareValue(PWM_0_INST,ABS(pwma),GPIO_PWM_0_C0_IDX);
	}
	else
	{
		DL_GPIO_setPins(AIN2_PORT,AIN1_PIN_12_PIN);
		DL_GPIO_clearPins(AIN1_PORT,AIN2_PIN_13_PIN);
		DL_Timer_setCaptureCompareValue(PWM_0_INST,ABS(pwma),GPIO_PWM_0_C0_IDX);
	}
	if(pwmb>0)
	{
